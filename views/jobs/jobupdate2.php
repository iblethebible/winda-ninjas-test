...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-sm">
                <?php
                $jobcompleteunpaid = '<form action="jobupdate2.php" method="get" onsubmit="return confirmCompletion(event, \'' . $dateNextDue . '\');"><input type="hidden" name="id" value="' . $job_id . '"><input id="unpaid-button" class="btn btn-warning" type="submit" name="submit_button" value="JOB COMPLETE/UNPAID">';
            ...
            <div class="col-sm">
                <?php
                $jobcompletepaid = '<form action="jobupdate2.php" method="get" onsubmit="return confirmCompletion(event, \'' . $dateNextDue . '\');"><input type="hidden" name="id" value="' . $job_id . '"><input class="btn btn-success" type="submit" name="submit_two" value="JOB COMPLETE/PAID"></form>';
...
<script type="text/javascript">
        let dateNextDue = "<?php echo $dateNextDue; ?>"; // Get date from PHP variable
        console.log("Global scope dateNextDue: " + dateNextDue);

        function confirmCompletion(event) {
            // Prevent form submission first
            event.preventDefault();
            console.log("Function confirmCompletion called.");

            // Try logging the variable here.
            console.log("dateNextDue within function: " + dateNextDue);

            var parts = dateNextDue.split('/');
            var formattedDateNextDue = new Date(parts[2], parts[1] - 1, parts[0]);
            formattedDateNextDue.setHours(0, 0, 0, 0);

            console.log("Formatted Date Next Due: " + formattedDateNextDue);
            ...
            const timeDifference = formattedDateNextDue - currentDate;
            const dayDifference = Math.floor(timeDifference / (1000 * 60 * 60 * 24));

            let shouldProceed = true;

            if (currentDate < formattedDateNextDue) {
                let earlyDaysMessage = dayDifference > 0 ? `This job is ${dayDifference} days early. Are you sure you want to mark it as complete?` : "This job is not due yet. Are you sure you want to mark it as complete?";
                shouldProceed = confirm(earlyDaysMessage);
            }

            if (shouldProceed) {
                // Log the message that shouldProceed is set to true
                console.log("shouldProceed is set to true");

                // Log the form to be submitted
                const formToSubmit = event.target.closest('form');
                console.log("Form to submit:", formToSubmit);

                // If conditions are met, submit the form manually
                formToSubmit.submit();
            }
